var setup_counterfactuals = function(prior_functions, meaning, utterancePrior, stickiness, rewrite_worlds_from_rvs_fn) {
  ...
  var cached_meaning = cache(meaning);
	...
	var literal_listener = cache(function(utterance, cf_premise_type, cf_conclusion_type) {
	  return safe_infer({method: "enumerate", maxExecutions: 10000}, function() {
	    var cf_premise_type = cf_premise_type || cf_type_sampler();
	    var cf_conclusion_type = cf_conclusion_type || cf_type_sampler();
	    var w = prior_sampler();
	    var cf_w = cf_sampler(w);
	    var factor = (cached_meaning(
	    	utterance,
	    ...
	    return {factor: factor, result: w};
	  });
	});

	var speaker = function(w, cf_premise_type, cf_conclusion_type) {
	...
	var endorsement = function(w, cf_premise_type, cf_conclusion_type) {
	  var s1 = speaker(w, cf_premise_type, cf_conclusion_type);

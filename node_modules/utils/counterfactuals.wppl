  ...
  var cached_meaning = cache(meaning);
	...
	var literal_listener = cache(function(utterance, cf_premise_type, cf_conclusion_type) {
	  return safe_infer({method: "enumerate", maxExecutions: 1000000}, function() {
	    ...
	    // var f = 0;
	    return {factor: f, result: w};
	  });
	});

	var speaker = cache(function(w, base_utterance, cf_premise_type_input, cf_conclusion_type_input) {
      ...
      // display(utterance);
	    ...
	    return {
	    	...
	    	result: {
		    	utterance: utterance,
		    	cf_premise_type: cf_premise_type,
		    	cf_conclusion_type: cf_conclusion_type
		    }
		  };
	  });
	});

	var endorsement = function(w, utterance, cf_premise_type, cf_conclusion_type) {
	  var s1 = speaker(
	  	w, utterance, cf_premise_type, cf_conclusion_type
	  );
	  var s1_utterance = marginal(s1);
	  return Math.exp(s1_utterance.score(utterance));
	}

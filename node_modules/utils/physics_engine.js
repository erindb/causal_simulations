...
var setup_physics = function(w, result_index) {
  ...
  // Add a circle
  objects_storage["ballAShape" + result_index] = new p2.Circle({width: 2, height: 1});
  objects_storage["ballABody" + result_index] = new p2.Body({
    id: "A",
    mass: 1,
    position: [w.A.position*4-2, 0],
    velocity: [10*w.A.velocity, 0]
  });
  objects_storage["ballABody" + result_index].addShape(objects_storage["ballAShape" + result_index]);
  objects_storage["world" + result_index].addBody(objects_storage["ballABody" + result_index]);

  // Add a circle
  objects_storage["ballBShape" + result_index] = new p2.Circle({width: 2, height: 1});
  objects_storage["ballBBody" + result_index] = new p2.Body({
    id: "B",
    mass: 1,
    position: [w.B.position*4-2, 0],
    velocity: [10*w.B.velocity, 0]
  });
  objects_storage["ballBBody" + result_index].addShape(objects_storage["ballBShape" + result_index]);
  objects_storage["world" + result_index].addBody(objects_storage["ballBBody" + result_index]);

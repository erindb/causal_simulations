...
var worlds = [];
var write = function(world, label, display) {
	worlds.push({
		"world": world,
		"label": label,
...
var write_file = function(filename, content) {
	fs.writeFile(filename, content, function(err, data) {});
};

var write_results = function(filename, s) {
	console.log(results_dir + filename);
	write_file(results_dir + filename, s)
};
...
var finish = function(output_file_tag) {
	...
	// write s to a file
	console.log(results_dir + output_js_filename);
	write_file(results_dir + output_js_filename, s)
	// fs.write_file("results/tmp.html", s)

	fs.readFile(modules_dir + "utils/results_template/results_template.html", 'utf8', function(err, data) {
		write_file(results_dir + output_html_filename, data.replace(

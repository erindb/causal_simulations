...
df %>%
  ...
  filter(dne=="DNEseparate") %>%
  mutate(utterance=ifelse(utterance=="AmovedB", "AexistingcausedBtomove",
                          char(utterance))) %>%
  filter(endorsement != 0) %>%
  ggplot(aes(x=paste(gloss, A_velocity, B_velocity), y=endorsement, fill=nspeeds)) +
  ...
  ylim(0, 1)
ggsave(results_dir("endorsement_by_velocity.png"), width=16, height=8)
```
...
- 8-speed version is running on `cocoserv2`.
